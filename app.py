...
def manifest():
    response = send_file(os.path.join(app.static_folder, 'manifest.json'), mimetype='application/manifest+json')
    response.headers['Cache-Control'] = 'public, max-age=86400'
    return response

# Serve service worker with correct headers to prevent caching issues
@app.route('/sw.js')
def service_worker():
...
# Serve assetlinks.json from /.well-known/ - REQUIRED for TWA to remove browser UI
@app.route('/.well-known/assetlinks.json')
def assetlinks():
    """
    Serves the Digital Asset Links file required by Trusted Web Activities.
    This proves you own the website and the associated Android app.
    The file is generated by PWABuilder and downloaded as assetlinks.json
    """
    assetlinks_path = os.path.join(app.static_folder, '.well-known', 'assetlinks.json')
    
    # Check if the file exists
    if os.path.exists(assetlinks_path):
        response = send_file(assetlinks_path, mimetype='application/json')
        response.headers['Cache-Control'] = 'public, max-age=86400'
        return response
    else:
        # Return a placeholder if file doesn't exist yet
        # This will be replaced with the real file once you generate the Android app
        return jsonify({
            'status': 'pending',
            'message': 'assetlinks.json not yet configured. Complete PWABuilder setup and place the file at static/.well-known/assetlinks.json'
        }), 404
